...
# # homebox-restore-data.sh Description
# Restores homebox's data directory from a backup set taken by the backups service.
...
# 3. **Stop Service**: homebox is stopped so nothing writes to the data directory.
...
# 5. **Start Service**: homebox is started again.
# Make it executable once: `chmod +x homebox-restore-data.sh`

APP_CONTAINER="$(docker compose -p homebox ps -q homebox)"
BACKUPS_CONTAINER="$(docker compose -p homebox ps -q backups)"
BACKUP_PATH="/srv/homebox/backups"
DB_NAME="homebox-database-backup"
DATA_NAME="homebox-data-backup"
