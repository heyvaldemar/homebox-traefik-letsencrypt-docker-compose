#!/usr/bin/env bash
# homebox-restore-data.sh [timestamp]
#
# Replaces Homebox's data with one backup set: the data archive and, for each
# SQLite database, the consistent copy the backups service took beside it.
#
#   ./homebox-restore-data.sh                      list the sets and ask
#   ./homebox-restore-data.sh YYYY-MM-DD_hh-mm     restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than homebox.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-homebox}"
STOP_SERVICES="homebox"                   # every service that writes what is restored
...
DBS="homebox.db"                              # SQLite files in /data, restored from their copies
...
DIR="$(env_of HOMEBOX_BACKUPS_PATH)"; DATA_NAME="$(env_of HOMEBOX_DATA_BACKUP_NAME)"
DB_NAME=""; [ -z "$DBS" ] || DB_NAME="$(env_of HOMEBOX_BACKUP_NAME)"
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; the data may be incomplete. Restore another set before using Homebox." >&2
  exit 1
fi
