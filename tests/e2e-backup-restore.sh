#!/bin/bash
# End-to-end tests for the homebox-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-homebox}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-homebox-traefik-letsencrypt-docker-compose.yml}"
...
: "${HOMEBOX_BACKUPS_PATH:=/srv/homebox/backups}"
: "${HOMEBOX_BACKUP_NAME:=homebox-database-backup}"
: "${HOMEBOX_DATA_BACKUP_NAME:=homebox-data-backup}"
: "${HOMEBOX_BACKUP_INTERVAL:=24h}"

BACKUPS_PATH="${HOMEBOX_BACKUPS_PATH%/}"
DB_PREFIX="${HOMEBOX_BACKUP_NAME}"
DATA_PREFIX="${HOMEBOX_DATA_BACKUP_NAME}"
INTERVAL="${HOMEBOX_BACKUP_INTERVAL}"
DB_FILE="homebox.db"
DB_BASE="homebox"
...
APP_CONTAINER="$(docker compose -f "$DOCKER_COMPOSE_FILE" -p "$COMPOSE_PROJECT_NAME" ps -aq homebox | head -n 1)"
...
test_restore_roundtrip() {
  ...
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./homebox-restore-data.sh "$stamp" > /dev/null \
    || { fail "./homebox-restore-data.sh failed"; return 1; }
  local exists
...
echo "=== homebox: backup/restore E2E tests ==="
